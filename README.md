...
<link rel="icon" href="https://www.dropbox.com/scl/fi/jxjgwcg2oa5ic0jrfx5s6/.png?rlkey=tpj00fxn8kqnlt5rb75l7wmu3&st=szhw5y8u&dl=0">
    ...
    <style>
	 /* ----------------------------------------------------- */
        ...
        body.bg-sakura { background-image: linear-gradient(135deg, #FFB7C5, #FFE4E1); } /* ★イベント */
...
<body>
    ...
    <div id="sidebar">
        <div id="data-ui">
            ...
            <div class="sidebar-label"><i class="fas fa-coins"></i> 所持金</div>
            
            <div class="sidebar-value" id="high-score-display" style="margin-top: 15px;">0</div>
            <div class="sidebar-label" id="high-score-label">T-30ハイスコア</div>
        </div>
        
        <button id="time-mode-btn" class="mode-btn active"><i class="far fa-clock"></i> 30秒モード</button>
        <button id="word-mode-btn" class="mode-btn"><i class="fas fa-keyboard"></i> 30単語モード</button>
        <button id="sudden-death-mode-btn" class="mode-btn"><i class="fas fa-skull-crossbones"></i> サドンデス</button>
        
        <button id="time-attack-mode-btn" class="mode-btn challenge-btn"><i class="fas fa-stopwatch"></i> TA (20単語)</button>
        <button id="accuracy-mode-btn" class="mode-btn challenge-btn"><i class="fas fa-crosshairs"></i> 精度 (1分)</button>
        <button id="english-mode-btn" class="mode-btn challenge-btn"><i class="fas fa-font"></i> 英単語モード</button>
        <button id="double-mode-btn" class="mode-btn challenge-btn"><i class="fas fa-clone"></i> ダブル</button>
        <button id="fill-mode-btn" class="mode-btn challenge-btn"><i class="fas fa-question"></i> 穴埋め</button>
        <button id="fast-mode-btn" class="mode-btn challenge-btn"><i class="fas fa-forward"></i> 早送り</button>

        <button id="news-btn" class="sidebar-btn" style="border-color: #2ecc71;"><i class="fas fa-bullhorn"></i> お知らせ</button>
        <button id="ranking-btn" class="sidebar-btn" style="border-color: #f1c40f;"><i class="fas fa-medal"></i> ランキング</button>
        <button id="skill-tree-btn" class="sidebar-btn" style="border-color: #9b59b6;"><i class="fas fa-sitemap"></i> スキル (<span id="skill-point-display">0</span>)</button>

        <button id="shop-btn" class="sidebar-btn"><i class="fas fa-shopping-cart"></i> ショップ</button>
        <button id="title-list-btn" class="sidebar-btn"><i class="fas fa-trophy"></i> 称号一覧</button>
        <button id="mission-btn" class="sidebar-btn"><i class="fas fa-tasks"></i> ミッション</button>
        <button id="achievement-btn" class="sidebar-btn"><i class="fas fa-award"></i> 実績</button>
        <button id="stats-btn" class="sidebar-btn"><i class="fas fa-chart-bar"></i> 統計</button>
        <button id="advanced-settings-btn" class="sidebar-btn"><i class="fas fa-cog"></i> 詳細設定</button>
        <button id="custom-mode-btn" class="mode-btn"><i class="fas fa-edit"></i> カスタム</button>

        <button id="code-btn" class="sidebar-btn" style="margin-top: 15px; border-color: #f39c12;"><i class="fas fa-key"></i> コード</button>
        ...
        <div id="auto-type-panel-container" style="display: none; width: 100%; margin-top: 15px; border: 2px solid #f39c12; border-radius: 8px; padding: 10px; box-sizing: border-box;">
            <div class="auto-type-section">
                <h3 style="margin-top: 0;"><i class="fas fa-robot"></i> 自動入力</h3>
                <button class="auto-type-btn toggle" id="auto-type-toggle">自動入力 OFF</button>
            </div>
            <div class="auto-type-section">
                <h3><i class="fas fa-tachometer-alt"></i> 間隔 (ms)</h3>
    ...
    <div id="end-screen-overlay" class="modal-overlay" style="display: none;">
        <div id="results-content">
            ...
            <div class="result-stat" id="end-total-typed">総タイプ数: <span>0</span></div>
            <div class="result-stat" id="end-score">スコア: <span>0</span></div>
            <button id="share-button" class="sidebar-btn" style="width: auto; background-color: #1DA1F2; margin-top: 10px; margin-bottom: 20px; display: inline-block; text-align: center;"><i class="fas fa-share-alt"></i> スコアをシェア</button>
            <button id="retry-button">リトライ / Spaceで開始</button>
        </div>
    </div>
    
    <div id="shop-overlay" class="modal-overlay" style="display: none;">
        <div id="shop-content" class="modal-content">
            <h2><i class="fas fa-shopping-cart"></i> ショップ</h2>
            <button class="close-btn" id="close-shop-btn">&times;</button>
            <div id="gacha-section">
                <h3><i class="fas fa-box-open"></i> ランダムボックス</h3>
                <p>レアなアイテムが手に入るかも？<span id="gacha-bonus-text" style="color: var(--color-correct); font-weight: bold; display: none;"> (レア率UP中!)</span></p>
                <button id="gacha-pull-btn" class="gacha-btn">1回引く (5,000 G)</button>
            </div>
            <h2><i class="fas fa-paint-brush"></i> アイテム一覧</h2>
    ...
    <div id="ranking-overlay" class="modal-overlay panel-modal" style="display: none;">
        <div id="ranking-content" class="modal-content">
            <h2><i class="fas fa-medal"></i> ランキング</h2>
    ...
    <div id="skill-tree-overlay" class="modal-overlay" style="display: none;">
        <div id="skill-tree-content" class="modal-content">
            <h2><i class="fas fa-sitemap"></i> スキルツリー</h2>
            <button class="close-btn" id="close-skill-tree-btn">&times;</button>
            
            <div id="skill-tree-header">
                <div>
                    所持スキルポイント: <span id="skill-points-available">0</span> SP
                    (転生Lv. <span id="skill-tree-prestige-level">0</span>)
                </div>
                <div id="preset-container">
                    ...
                    <button id="save-preset-btn" class="sidebar-btn" style="width: auto; padding: 5px 10px; margin-left: 10px;"><i class="fas fa-save"></i> 保存</button>
                </div>
                <div>
                    <button id="skill-reset-btn" class="sidebar-btn" style="width: auto; background-color: var(--color-error);"><i class="fas fa-undo"></i> スキルリセット (P.Lv x 1000 G)</button>
                </div>
            </div>
    ...
    <div id="achievement-overlay" class="modal-overlay" style="display: none;">
        <div id="achievement-content" class="modal-content">
            <h2><i class="fas fa-award"></i> 実績</h2>
            <button class="close-btn" id="close-achievement-btn">&times;</button>
            <div id="ap-summary">
                累計実績ポイント: <span id="total-ap-display">0</span> AP
            </div>
    ...
    <div id="prestige-choice-overlay" class="modal-overlay panel-modal" style="display: none;">
        <div id="prestige-choice-content" class="modal-content" style="text-align: center;">
            <h2><i class="fas fa-star"></i> プレステージ Lv. <span id="prestige-modal-level">1</span></h2>
            <button class="close-btn" id="close-prestige-choice-btn">&times;</button>
            
            <div id="prestige-type-choice" style="display: none;">
                <p>最初の転生です。どちらの道を選びますか？ (この選択は永続です)</p>
                <div id="prestige-choice-container">
                    <div class="prestige-choice-option" id="prestige-choice-standard">
                        <h3><i class="fas fa-tachometer-alt"></i> スタンダード転生</h3>
                        <p>スコアとXPの獲得量に永久ボーナスを得る、王道の転生です。</p>
                    </div>
                    <div class="prestige-choice-option" id="prestige-choice-wealth">
                        <h3><i class="fas fa-gem"></i> 富豪の転生</h3>
                        <p>ガチャのレアアイテム排出率が永続的に上昇する、コレクター向けの転生です。</p>
                    </div>
                </div>
            </div>

            <div id="prestige-trial-choice" style="display: none;">
                <p>転生の「試練」を選択してください。試練を達成して転生すると、獲得スキルポイントが増加します。</p>
                <div id="prestige-trial-container">
                    <div class="prestige-choice-option" id="prestige-trial-none">
                        <h3><i class="fas fa-running"></i> 試練なし (x1.0)</h3>
                        <p>試練を行わず、通常量のスキルポイント（<span class="trial-sp-reward">1</span> SP）を獲得して転生します。</p>
                    </div>
                    <div class="prestige-choice-option" id="prestige-trial-combo">
                        <h3><i class="fas fa-link"></i> コンボの試練 (x2.0)</h3>
                        <p>「転生後、Lv. 30になるまでに**100コンボ**を達成する」</p>
                        <p>達成で <span class="trial-sp-reward">2</span> SP 獲得。</p>
                    </div>
                    <div class="prestige-choice-option" id="prestige-trial-perfect">
                        <h3><i class="fas fa-check-double"></i> 完全無欠の試練 (x3.0)</h3>
                        <p>「転生後、Lv. 30になるまでに**30秒モードをノーミス**でクリアする」</p>
                        <p>達成で <span class="trial-sp-reward">3</span> SP 獲得。</p>
                    </div>
                </div>
                <div id="prestige-current-trial" style="margin-top: 20px;">
                    <h4><i class="fas fa-tasks"></i> 現在の試練</h4>
                    <p id="prestige-current-trial-text">なし</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="settings-overlay" class="modal-overlay" style="display: none;">
        <div id="settings-content" class="modal-content">
            <h2><i class="fas fa-cog"></i> 詳細設定</h2>
            <button class="close-btn" id="close-settings-btn">&times;</button>
            <div style="margin-top: 20px;">
                <button id="pop-mode-btn" class="sidebar-btn" style="width: auto;"><i class="fas fa-bolt"></i> 演出: ランダム</button>
                
                <button id="furigana-toggle-btn" class="sidebar-btn" style="width: auto; margin-top: 10px;">
                    <i class="fas fa-eye"></i> ふりがな: ON
                </button>
                ...
                <button id="legend-effect-btn" class="sidebar-btn" style="display: none; width: auto; margin-top: 10px; border-color: #f1c40f;">
                    <i class="fas fa-magic"></i> 伝説エフェクト: ON
                </button>

                <h3 style="margin-top: 30px;">お守り装備</h3>
                <div id="charm-selector-container"></div>

                <h3 style="margin-top: 30px;">テーマ (スキン)</h3>
                <div id="theme-selector-container"></div>
                
                <h3 style="margin-top: 30px;">ペット設定</h3>
                <div id="pet-selector-container"></div>
                <h4 style="margin-top: 15px;">ペット着せ替え</h4>
                <div id="pet-item-selector-container"></div>
                <h4 style="margin-top: 15px;">ペットの家</h4>
                <div id="pet-house-selector-container"></div>
                
                <h3 style="margin-top: 30px;">プレイヤーアイコン</h3>
                <div id="icon-selector-container"></div>

                <h3 style="margin-top: 30px;">ポップエフェクト変更</h3>
                <div id="effect-selector-container"></div>
                
                <h3 style="margin-top: 30px;">単語パック</h3>
                <div id="word-pack-selector-container"></div>

                <h3 style="margin-top: 30px;">フォント変更</h3>
                <div id="font-selector-container"></div>
                
                <h3 style="margin-top: 30px;">タイプ音変更</h3>
                <div id="sound-selector-container"></div>
                
                <h3 style="margin-top: 30px;">BGM変更 (ジュークボックス)</h3>
                <div id="bgm-selector-container"></div>

                <h3 style="margin-top: 30px;">キーボードスキン変更</h3>
                <div id="keyboard-skin-selector-container"></div>

                <h3 style="margin-top: 30px;">背景テーマ</h3>
                <div id="background-selector-container"></div> 
                
                <h3 style="margin-top: 30px;">練習</h3>
                <div id="practice-container">
                    <button id="weak-word-practice-btn" class="sidebar-btn" style="width: auto;"><i class="fas fa-dumbbell"></i> 苦手単語練習</button>
                </div>

                <h3 style="margin-top: 30px;">データ管理</h3>
                <div class="data-management-grid">
                    <button id="export-button" class="sidebar-btn" style="width: auto; background-color: #f39c12;"><i class="fas fa-download"></i> データ書き出し (.sav)</button>
                    <button onclick="document.getElementById('import-input').click()" class="sidebar-btn" style="width: auto; background-color: #3498db;"><i class="fas fa-upload"></i> データ読み込み</button>
                    <button id="delete-data-btn" class="sidebar-btn" style="width: auto; background-color: #e74c3c;"><i class="fas fa-trash-alt"></i> 全データ削除</button>
                </div>
                <input type="file" id="import-input" accept=".sav" style="display: none;">
                
                <div id="custom-word-section" style="margin-top: 20px;">
                    <h4>カスタムお題（「漢字[よみ]」形式で入力）</h4>
                    <textarea id="custom-textarea" rows="5" style="width: 100%; box-sizing: border-box; font-family: var(--font-main); font-size: 1em; background: var(--color-bg-start); color: var(--color-text); border: 1px solid var(--color-border); border-radius: 5px;" placeholder="例：日本[にほん]&#10;寿司[すし]&#10;タイピング"></textarea>
                    <button id="start-custom-btn" class="sidebar-btn" style="width: auto; background-color: var(--color-correct); margin-top: 10px;"><i class="fas fa-play"></i> このお題で開始</button>
                </div>
            
                <h3 style="margin-top: 30px;">プレステージ</h3>
                <button id="prestige-btn" class="sidebar-btn" style="width: 100%; background-color: #e67e22; border-color: #f39c12; display: none;"><i class="fas fa-star"></i> プレステージ (Lv. 100で実行可)</button>
            </div>
        </div>
    </div>
    ...
    <div id="admin-panel-overlay" class="modal-overlay panel-modal" style="display: none;">
        <div id="admin-panel-content" class="modal-content">
            <h2><i class="fas fa-cogs"></i> 管理者パネル</h2>
            <button class="close-btn" id="close-admin-panel-btn">&times;</button>
            <p>セーブデータの値を直接変更します。</p>

            <div id="admin-panel-container">
                <div class="admin-panel-section">
                    <h3><i class="fas fa-coins"></i> 所持金</h3>
                    <div class="admin-grid-money">
                        ...
                        <button class="admin-btn reset" data-action="money" data-value="reset">所持金を 0 にする</button>
                    </div>
                </div>

                <div class="admin-panel-section">
                    <h3><i class="fas fa-level-up-alt"></i> レベル & XP</h3>
                    <div class="admin-grid-unlocks">
                        ...
                        <button class="admin-btn action" data-action="level" data-value="100">Lv. 100 に設定</button>
                        <button class="admin-btn reset" data-action="level" data-value="reset">Lv. 1 にリセット</button>
                    </div>
                </div>
                
                <div class="admin-panel-section">
                    <h3><i class="fas fa-sitemap"></i> スキル</h3>
                    <div class="admin-grid-unlocks">
                ...
                <div class="admin-panel-section">
                    <h3><i class="fas fa-unlock-alt"></i> 解放設定</h3>
                    <div class="admin-grid-unlocks">
                        <button class="admin-btn action" data-action="unlock-all">全アイテム/パック解放</button>
                        <button class="admin-btn action" data-action="lock-all">全アイテム/パック削除</button>
                        <button class="admin-btn action" data-action="unlock-titles">称号 全獲得</button>
                        <button class="admin-btn action" data-action="lock-titles">称号 全削除</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
/* ----------------------------------------------------- */
        /* JavaScript: ゲームロジック (完全修正版 v14) */
        /* ----------------------------------------------------- */
        
        // --- 1. 定数とDOM要素の取得 ---
        const STORAGE_KEY = 'pop-typer-data-v14-full-bugfix'; // バージョンアップ
        const TOTAL_TITLES = 10;
        const XP_PER_CHAR = 1; // 1文字あたりのXP
        const XP_PER_WORD = 10; // 1単語クリアあたりのXP
        const LEVEL_BASE_XP = 100; // レベル1→2に必要なXP
        const MAX_LEVEL = 100; // プレステージ可能なレベル
        const MAX_PRESTIGE_LEVEL = 20; // ★転生レベル上限
        ...
        const wordDisplaySub = document.getElementById('word-display-sub'); // ★ダブルモード
        ...
        let lastGameResult = {}; 
        let quickClaimQueue = []; // ミッション即時受け取り用
        ...
        // 新モード
        let doubleWordData = null; // ダブルモードのサブお題
        let fastModeSpeed = 1.0; // 早送りモードの速度
        
        // --- 3. データ定義 (ショップアイテム / 称号 / お題) ---
        const masterItemList = {
            // ブースト
            scoreBoost: { name: "スコアブースト", price: 250, description: "次のゲームのスコアを2倍にします。", type: "boost" },
            moneyBoost: { name: "お金ブースト", price: 200, description: "次のゲームの獲得お金を2倍にします。", type: "boost" },
            comboShield: { name: "コンボシールド", price: 300, description: "次のゲームで1回だけミスを無効化します。", type: "boost" },
            easyStart: { name: "イージースタート", price: 150, description: "次のゲームの最初の5単語が簡単になります。", type: "boost" },
            boostStart: { name: "ブーストスタート", price: 250, description: "コンボ10からゲームを開始します。", type: "boost" },
            timePlusTwo: { name: "タイムプラス (+2秒)", price: 150, description: "時間モードの制限時間を2秒延長します。", type: "boost" },
            timePlusFive: { name: "タイムプラス (+5秒)", price: 300, description: "時間モードの制限時間を5秒延長します。", type: "boost" },
            wordSkip: { name: "単語スキップ", price: 150, description: "単語モードのノルマを1単語減らします。", type: "boost" },
            slowTimer: { name: "スロータイマー", price: 280, description: "タイマーの進み方が10%遅くなります。", type: "boost" },
            feverBoost: { name: "フィーバー+", price: 500, description: "10コンボでフィーバータイム(スコア/金3倍)が発動。", type: "boost" }, 
            bonusMoney: { name: "ボーナスマネー", price: 150, description: "購入後すぐに50円を獲得します。", type: "utility" },
            
            // アップグレード
            upgrade_money: { name: "獲得金額UP (永続)", price: 5000, description: "ゲーム終了時の獲得金額が永久に10%増加します。", type: "upgrade", key: "moneyBoost" },
            upgrade_fever: { name: "フィーバー時間UP (永続)", price: 8000, description: "フィーバータイムの持続時間が永久に1秒増加します。", type: "upgrade", key: "feverTime" },
            upgrade_infiniteFever: { name: "無限フィーバー (永続)", price: 100000, description: "フィーバータイムがミスをしてもゲーム終了まで永続します。", type: "upgrade", key: "infiniteFever" },

            // 背景
            bg_ocean: { name: "背景: 海", price: 1000, description: "背景テーマ「海」を解放します。", type: "background" },
            bg_galaxy: { name: "背景: 銀河", price: 1500, description: "背景テーマ「銀河」を解放します。", type: "background" },
            bg_forest: { name: "背景: 森", price: 1000, description: "背景テーマ「森」を解放します。", type: "background" },
            bg_sunset: { name: "背景: 夕焼け", price: 1200, description: "背景テーマ「夕焼け」を解放します。", type: "background" },
            bg_cyber: { name: "背景: サイバー", price: 2000, description: "背景テーマ「サイバー」を解放します。", type: "background" },
            bg_legend: { name: "背景: 伝説", price: 50000, description: "伝説の背景テーマを解放します。", type: "background" },
            bg_sakura: { name: "背景: 桜", price: 0, description: "イベント限定の背景です。", type: "background", event: "e_sakura" }, // ★イベント

            // フォント
            font_serif: { name: "フォント: 明朝体", price: 3000, description: "ゲーム全体のフォントを明朝体に設定できます。", type: "font" },
            font_rounded: { name: "フォント: 丸ゴシック", price: 3000, description: "ゲーム全体のフォントを丸ゴシック体に設定できます。", type: "font" },

            // タイプ音
            sound_click: { name: "タイプ音: カチカチ", price: 2000, description: "タイプ音をメカニカル風にします。", type: "sound" },
            sound_synth: { name: "タイプ音: ピコピコ", price: 2000, description: "タイプ音を電子音風にします。", type: "sound" },
            
            // 単語パック
            pack_animal: { name: "単語パック: 動物", price: 5000, description: "動物に関連する単語が出題されるようになります。", type: "pack" },
            pack_food: { name: "単語パック: 食べ物", price: 5000, description: "食べ物に関連する単語が出題されるようになります。", type: "pack" },
            pack_difficult: { name: "単語パック: 神々", price: 50000, description: "超高難易度の単語パックです。", type: "pack" },

            // ポップエフェクト
            effect_rainbow: { name: "エフェクト: 虹色", price: 10000, description: "タイプ文字が虹色に光ります。", type: "effect" },
            effect_firework: { name: "エフェクト: 花火", price: 15000, description: "タイプ文字が花火のように爆発します。", type: "effect" },
            effect_bubble: { name: "エフェクト: 泡", price: 12000, description: "タイプ文字が泡になって消えます。", type: "effect" },
            effect_lines: { name: "エフェクト: 集中線", price: 12000, description: "タイプ文字に集中線が出ます。", type: "effect" },

            // テーマ
            theme_cyber: { name: "テーマ: サイバー", price: 30000, description: "UI全体をサイバーパンク風にします。", type: "theme" },
            theme_wa: { name: "テーマ: 和風", price: 30000, description: "UI全体を和風（明色）にします。", type: "theme" },
            theme_pixel: { name: "テーマ: ピクセル", price: 20000, description: "UI全体をレトロゲーム風にします。", type: "theme" },
            
            // ペット
            pet_dog: { name: "ペット: 犬", price: 25000, description: "マスコット「犬」を解放します。", type: "pet" },
            pet_robot: { name: "ペット: ロボット", price: 25000, description: "マスコット「ロボット」を解放します。", type: "pet" },
            pet_item_santa: { name: "ペット服: サンタ帽", price: 10000, description: "ペット用のサンタ帽です。", type: "pet-item" },
            pet_house_cushion: { name: "ペットの家: クッション", price: 15000, description: "ペットの足場をクッションにします。", type: "pet-house" },
            pet_house_space: { name: "ペットの家: 宇宙船", price: 15000, description: "ペットの足場を宇宙船にします。", type: "pet-house" },
            
            // BGM (ジュークボックス)
            bgm_2: { name: "BGM: Battle", price: 10000, description: "BGM「Battle」を解放します。", type: "bgm" },
            bgm_3: { name: "BGM: Dream", price: 10000, description: "BGM「Dream」を解放します。", type: "bgm" },
            
            // キーボードスキン
            kb_skin_wood: { name: "KBスキン: 木目調", price: 8000, description: "キーボードを木目調にします。", type: "kb-skin" },
            kb_skin_pixel: { name: "KBスキン: ピクセル", price: 8000, description: "キーボードをレトロ風にします。", type: "kb-skin" },
            
            // プレイヤーアイコン
            icon_neko: { name: "アイコン: 猫", price: 5000, description: "プロフィールアイコン「😺」", type: "icon", emoji: "😺" },
            icon_inu: { name: "アイコン: 犬", price: 5000, description: "プロフィールアイコン「🐶」", type: "icon", emoji: "🐶" },
            icon_default: { name: "アイコン: デフォルト", price: 0, description: "プロフィールアイコン「👤」", type: "icon", emoji: "👤" },

            // ★ガチャ限定アイテム
            effect_gold: { name: "エフェクト: ゴールド", description: "タイプ文字が金色に輝く！", type: "effect", gacha: true, rarity: "super-rare" },
            bg_aurora: { name: "背景: オーロラ", description: "幻想的なオーロラの背景", type: "background", gacha: true, rarity: "super-rare" },
            pet_item_party: { name: "ペット服: パーティ帽", description: "ペット用のお祝いの帽子", type: "pet-item", gacha: true, rarity: "rare" },
            pet_item_sakura: { name: "ペット服: 桜", description: "ペット用の桜のかんざし", type: "pet-item", gacha: true, rarity: "rare", event: "e_sakura" }, // ★イベント
            icon_legend: { name: "アイコン: 伝説", description: "プロフィールアイコン「✨」", type: "icon", gacha: true, rarity: "super-rare", emoji: "✨" },
            kb_skin_neko: { name: "KBスキン: 肉球", description: "キーボードが肉球まみれに", type: "kb-skin", gacha: true, rarity: "rare" },
            charm_maneki: { name: "お守り: 金の招き猫", description: "獲得Gが永久に+5%！ (装備中のみ)", type: "charm", gacha: true, rarity: "legendary" },
            charm_kame: { name: "お守り: 亀の甲羅", description: "最初の3ミスまでコンボが途切れない (装備中のみ)", type: "charm", gacha: true, rarity: "legendary" },
            charm_taka: { name: "お守り: 鷹の羽", description: "ポップエフェクトが少し豪華になる (装備中のみ)", type: "charm", gacha: true, rarity: "rare" },
            charm_hude: { name: "お守り: 達人の筆", description: "WPMに応じてスコアにボーナス (装備中のみ)", type: "charm", gacha: true, rarity: "legendary" },
            money_small: { name: "500 G", description: "500 G を獲得した", type: "utility", gacha: true, rarity: "normal", value: 500 },
            money_medium: { name: "10,000 G", description: "10,000 G を獲得した！", type: "utility", gacha: true, rarity: "rare", value: 10000 },
            boost_pack: { name: "ブーストパック", description: "ブーストアイテム3種詰め合わせ！", type: "utility", gacha: true, rarity: "rare", value: ['scoreBoost', 'moneyBoost', 'comboShield'] }
        };
        ...
        const masterTitleList = Array(TOTAL_TITLES).fill(0).map((_i, i) => ({ name: `達人 ${i + 1} 段`, score: (i + 1) * 50000 }));
        
        const wordPacks = {
            'default': [
                { display: "<ruby>猫<rt>ねこ</rt></ruby>", yomi: "ねこ" }, { display: "<ruby>犬<rt>いぬ</rt></ruby>", yomi: "いぬ" }, { display: "<ruby>空<rt>そら</rt></ruby>", yomi: "そら" }, { display: "<ruby>海<rt>うみ</rt></ruby>", yomi: "うみ" }, { display: "<ruby>夢<rt>ゆめ</rt></ruby>", yomi: "ゆめ" },
                { display: "<ruby>山<rt>やま</rt></ruby>", yomi: "やま" }, { display: "<ruby>川<rt>かわ</rt></ruby>", yomi: "かわ" }, { display: "<ruby>月<rt>つき</rt></ruby>", yomi: "つき" }, { display: "<ruby>星<rt>ほし</rt></ruby>", yomi: "ほし" }, { display: "<ruby>雨<rt>あめ</rt></ruby>", yomi: "あめ" },
                { display: "<ruby>仕事<rt>しごと</rt></ruby>", yomi: "しごと" }, { display: "<ruby>勉強<rt>べんきょう</rt></ruby>", yomi: "べんきょう" },
                { display: "ゲーム", yomi: "げーむ" }, { display: "コード", yomi: "こーど" },
                { display: "パソコン", yomi: "ぱそこん" }, { display: "<ruby>学校<rt>がっこう</rt></ruby>", yomi: "がっこう" }, { display: "<ruby>会社<rt>かいしゃ</rt></ruby>", yomi: "かいしゃ" }, { display: "おはよう", yomi: "おはよう" }, { display: "<ruby>元気<rt>げんき</rt></ruby>", yomi: "げんき" },
                { display: "キーボード", yomi: "きーぼーど" }, { display: "<ruby>一日<rt>いちにち</rt></ruby>", yomi: "いちにち" }, { display: "<ruby>挨拶<rt>あいさつ</rt></ruby>", yomi: "あいさつ" }, { display: "<ruby>友達<rt>ともだち</rt></ruby>", yomi: "ともだち" }, { display: "タイピング", yomi: "たいぴんぐ" },
                { display: "<ruby>日本<rt>にほん</rt></ruby>", yomi: "にほん" }, { display: "<ruby>世界<rt>せかい</rt></ruby>", yomi: "せかい" }, { display: "<ruby>未来<rt>みらい</rt></ruby>", yomi: "みらい" },
                { display: "ありがとう", yomi: "ありがとう" }, { display: "こんにちは", yomi: "こんにちは" }, { display: "さようなら", yomi: "さようなら" }, { display: "はじめまして", yomi: "はじめまして" },
                { display: "<ruby>お疲れ様<rt>おつかれさま</rt></ruby>", yomi: "おつかれさま" }, { display: "プログラミング", yomi: "ぷろぐらみんぐ" }, { display: "コンボボーナス", yomi: "こんぼぼーなす" },
                { display: "<ruby>動物園<rt>どうぶつえん</rt></ruby>", yomi: "どうぶつえん" }, { display: "<ruby>水族館<rt>すいぞくかん</rt></ruby>", yomi: "すいぞくかん" }, { display: "マウスクリック", yomi: "まうすくりっく" }, { display: "<ruby>新幹線<rt>しんかんせん</rt></ruby>", yomi: "しんかんせん" },
                { display: "<ruby>電光石火<rt>でんこうせっか</rt></ruby>", yomi: "でんこうせっか" }, { display: "<ruby>春夏秋冬<rt>しゅんかしゅうとう</rt></ruby>", yomi: "しゅんかしゅうとう" }, { display: "<ruby>温故知新<rt>おんこちしん</rt></ruby>", yomi: "おんこちしん" }, { display: "インターネット", yomi: "いんたーねっと" },
                { display: "<ruby>一期一会<rt>いちごいちえ</rt></ruby>", yomi: "いちごいちえ" }, { display: "<ruby>七転八起<rt>しちてんはっき</rt></ruby>", yomi: "しちてんはっき" }, { display: "<ruby>弱肉強食<rt>じゃにくきょうしょく</rt></ruby>", yomi: "じゃにくきょうしょく" }, { display: "<ruby>大器晩成<rt>たいきばんせい</rt></ruby>", yomi: "たいきばんせい" },
                { display: "フィーバータイム", yomi: "ふぃーばーたいむ" }, { display: "<ruby>自動<rt>じどう</rt></ruby><ruby>入力<rt>にゅうりょく</rt></ruby>", yomi: "じどうにゅうりょく" }, { display: "タイピング<ruby>練習<rt>れんしゅう</rt></ruby>", yomi: "たいぴんぐれんしゅう" },
                { display: "スマートフォン", yomi: "すまーとふぉん" }, { display: "<ruby>一日一善<rt>いちにちいちぜん</rt></ruby>", yomi: "いちにちいちぜん" }, { display: "<ruby>百花繚乱<rt>ひゃっかりょうらん</rt></ruby>", yomi: "ひゃっかりょうらん" },
                { display: "<ruby>一攫千金<rt>いっかくせんきん</rt></ruby>", yomi: "いっかくせんきん" }, { display: "<ruby>油断大敵<rt>ゆだんたいてき</rt></ruby>", yomi: "ゆだんたいてき" }, { display: "<ruby>不言実行<rt>ふげんじっこう</rt></ruby>", yomi: "ふげんじっこう" }, 
                { display: "<ruby>月<rt>つき</rt></ruby>が<ruby>綺麗<rt>きれい</rt></ruby>ですね", yomi: "つきがきれいですね" }, { display: "サーバーダウン", yomi: "さーばーだうん" }, { display: "<ruby>人工<rt>じんこう</rt></ruby><ruby>知能<rt>ちのう</rt></ruby>", yomi: "じんこうちのう" },
                { display: "<ruby>吾輩<rt>わがはい</rt></ruby>は<ruby>猫<rt>ねこ</rt></ruby>である", yomi: "わがはいはねこである" },
                { display: "クラウドコンピューティング", yomi: "くらうどこんぴゅーてぃんぐ" },
                { display: "<ruby>祇園精舎<rt>ぎおんしょうじゃ</rt></ruby>の<ruby>鐘<rt>かね</rt></ruby>の<ruby>声<rt>こえ</rt></ruby>", yomi: "ぎおんしょうじゃのかねのこえ" },
                { display: "デジタルトランスフォーメーション", yomi: "でじたるとらんすふぉーめーしょん" },
                { display: "<ruby>生<rt>なま</rt></ruby><ruby>麦<rt>むぎ</rt></ruby><ruby>生<rt>なま</rt></ruby><ruby>米<rt>ごめ</rt></ruby><ruby>生<rt>なま</rt></ruby><ruby>卵<rt>たまご</rt></ruby>", yomi: "なまむぎなまごめなまたまご" },
                { display: "オブジェクト<ruby>指向<rt>しこう</rt></ruby>プログラミング", yomi: "おぶじぇくとしこうぷろぐらみんぐ" },
                { display: "<ruby>日本<rt>にほん</rt></ruby><ruby>経済<rt>けいざい</rt></ruby><ruby>新聞<rt>しんぶん</rt></ruby>", yomi: "にほんけいざいしんぶん" },
                { display: "<ruby>隣<rt>となり</rt></ruby>の<ruby>客<rt>きゃく</rt></ruby>はよく<ruby>柿<rt>かき</rt></ruby><ruby>食<rt>く</rt></ruby>う<ruby>客<rt>きゃく</rt></ruby>だ", yomi: "となりのきゃくはよくかきくうきゃくだ" },
                { display: "バスガス<ruby>爆発<rt>ばくはつ</rt></ruby>", yomi: "ばすがすばくはつ" },
                { display: "<ruby>赤<rt>あか</rt></ruby>パジャマ<ruby>青<rt>あお</rt></ruby>パジャマ<ruby>黄<rt>き</rt></ruby>パジャマ", yomi: "あかぱじゃまあおぱじゃまきぱじゃま" },
                { display: "この<ruby>中<rt>なか</rt></ruby>に<ruby>裏切<rt>うらぎ</rt></ruby>り<ruby>者<rt>もの</rt></ruby>がいる", yomi: "このなかにうらぎりものがいる" },
                { display: "<ruby>俺<rt>おれ</rt></ruby>の<ruby>戦<rt>たたか</rt></ruby>いはこれからだ", yomi: "おれのたたかいはこれからだ" },
                { display: "メリークリスマス", yomi: "めりーくりすます" },
                { display: "あけましておめでとう", yomi: "あけましておめでとう" },
                { display: "JavaScript", yomi: "じゃばすくりぷと" }, { display: "Python", yomi: "ぱいそん" },
                { display: "CSS", yomi: "しーえすえす" }, { display: "HTML", yomi: "えいちてぃーえむえる" },
                { display: "データベース", yomi: "でーたべーす" }, { display: "ファイアウォール", yomi: "ふぁいあうぉーる" },
                { display: "セキュリティ", yomi: "せきゅりてぃ" }
            ],
            'pack_animal': [
                { display: "<ruby>猫<rt>ねこ</rt></ruby>", yomi: "ねこ" }, { display: "<ruby>犬<rt>いぬ</rt></ruby>", yomi: "いぬ" }, { display: "ライオン", yomi: "らいおん" }, { display: "キリン", yomi: "きりん" },
                { display: "パンダ", yomi: "ぱんだ" }, { display: "コアラ", yomi: "こあら" }, { display: "ペンギン", yomi: "ぺんぎん" },
                { display: "カピバラ", yomi: "かぴばら" }, { display: "アルパカ", yomi: "あるぱか" }, { display: "フェネック", yomi: "ふぇねっく" },
                { display: "シマエナガ", yomi: "しまえなが" }, { display: "ティラノサウルス", yomi: "てぃらのさうるす" },
                { display: "<ruby>兎<rt>うさぎ</rt></ruby>", yomi: "うさぎ" }, { display: "<ruby>象<rt>ぞう</rt></ruby>", yomi: "ぞう" }, { display: "<ruby>虎<rt>とら</rt></ruby>", yomi: "とら" }, { display: "<ruby>馬<rt>うま</rt></ruby>", yomi: "うま" },
                { display: "<ruby>羊<rt>ひつじ</rt></ruby>", yomi: "ひつじ" }, { display: "<ruby>猿<rt>さる</rt></ruby>", yomi: "さる" }, { display: "<ruby>鼠<rt>ねずみ</rt></ruby>", yomi: "ねずみ" }, { display: "<ruby>牛<rt>うし</rt></ruby>", yomi: "うし" },
                { display: "カバ", yomi: "かば" }, { display: "サイ", yomi: "さい" }, { display: "シマウマ", yomi: "しまうま" }, { display: "カンガルー", yomi: "かんがるー" },
                { display: "ゴリラ", yomi: "ごりら" }, { display: "チンパンジー", yomi: "ちんぱんじー" }, { display: "カワウソ", yomi: "かわうそ" }
            ],
            'pack_food': [
                { display: "<ruby>寿司<rt>すし</rt></ruby>", yomi: "すし" }, { display: "ラーメン", yomi: "らーめん" }, { display: "<ruby>天ぷら<rt>てんぷら</rt></ruby>", yomi: "てんぷら" },
                { display: "バナナ", yomi: "ばなな" }, { display: "りんご", yomi: "りんご" }, { display: "メロン", yomi: "めろん" },
                { display: "おにぎり", yomi: "おにぎり" }, { display: "ハンバーグ", yomi: "はんばーぐ" },
                { display: "<ruby>卵焼<rt>たまごや</rt></ruby>き", yomi: "たまごやき" }, { display: "お<ruby>寿司<rt>すし</rt></ruby>やさん", yomi: "おすしやさん" },
                { display: "<ruby>焼肉<rt>やきにく</rt></ruby><ruby>定食<rt>ていしょく</rt></ruby>", yomi: "やきにくていしょく" }, { display: "<ruby>豚<rt>ぶた</rt></ruby>バラブロック", yomi: "ぶたばらぶろっく" },
                { display: "カレーライス", yomi: "かれーらいす" }, { display: "<ruby>焼<rt>や</rt></ruby>き<ruby>肉<rt>にく</rt></ruby>食べ<ruby>放題<rt>ほうだい</rt></ruby>", yomi: "やきにくたべほうだい" },
                { display: "シュークリーム", yomi: "しゅーくりーむ" }, { display: "エビチリソース", yomi: "えびちりそーす" },
                { display: "<ruby>麻婆豆腐<rt>まーぼーどうふ</rt></ruby>", yomi: "まーぼーどうふ" }, { display: "オムライス", yomi: "おむらいす" },
                { display: "ナポリタン", yomi: "なぽりたん" }, { display: "いちご", yomi: "いちご" }, { display: "ぶどう", yomi: "ぶどう" },
                { display: "みかん", yomi: "みかん" }, { display: "ショートケーキ", yomi: "しょーとけーき" }, { display: "モンブラン", yomi: "もんぶらん" }
            ],
            'pack_difficult': [ // ★新パック
                { display: "<ruby>魑魅魍魎<rt>ちみもうりょう</rt></ruby>", yomi: "ちみもうりょう" },
                { display: "<ruby>檸檬<rt>れもん</rt></ruby>", yomi: "れもん" },
                { display: "<ruby>薔薇<rt>ばら</rt></ruby>", yomi: "ばら" },
                { display: "<ruby>憂鬱<rt>ゆううつ</rt></ruby>", yomi: "ゆううつ" },
                { display: "<ruby>挨拶<rt>あいさつ</rt></ruby>", yomi: "あいさつ" },
                { display: "<ruby>醤油<rt>しょうゆ</rt></ruby>", yomi: "しょうゆ" },
                { display: "<ruby>御御御付<rt>おみおつけ</rt></ruby>", yomi: "おみおつけ" }
            ],
            // ★歌詞タイピング削除
            'pack_english': [ // ★新パック
                { display: "<ruby>猫<rt>ねこ</rt></ruby>", yomi: "cat" },
                { display: "<ruby>犬<rt>いぬ</rt></ruby>", yomi: "dog" },
                { display: "<ruby>りんご<rt>りんご</rt></ruby>", yomi: "apple" },
                { display: "<ruby>水<rt>みず</rt></ruby>", yomi: "water" },
                { display: "<ruby>学校<rt>がっこう</rt></ruby>", yomi: "school" },
                { display: "<ruby>タイピング<rt>たいぴんぐ</rt></ruby>", yomi: "typing" },
                { display: "ゲーム", yomi: "game" }
            ],
            'pack_event_sakura': [ // ★イベント
                { display: "<ruby>桜<rt>さくら</rt></ruby>", yomi: "さくら" },
                { display: "<ruby>お花見<rt>おはなみ</rt></ruby>", yomi: "おはなみ" },
                { display: "スプリング", yomi: "すぷりんぐ" }
            ]
        };
        ...
        const masterMissionList = {
            dailyPlay: { name: "デイリープレイ", description: "1日に3回プレイする", goal: 3, reward: 100, type: "daily", key: "playCount" },
            dailyMoney: { name: "デイリー稼ぎ", description: "1日で合計1000 G稼ぐ", goal: 1000, reward: 200, type: "daily", key: "moneyEarned" },
            dailyTypes: { name: "デイリータイプ", description: "1日に合計1000回タイプする", goal: 1000, reward: 150, type: "daily", key: "totalTypes" },
            dailyMaxCombo: { name: "デイリーコンボ", description: "1日に最大コンボ30を達成する", goal: 30, reward: 150, type: "daily", key: "maxCombo" }
        };
        
        // ★ウィークリーミッション
        const masterWeeklyMissions = [
            { id: "w01", name: "動物愛", description: "動物パックで合計10,000点稼ぐ", goal: 10000, reward: 1000, check: (res, data) => (data.gameMode === 'time_30' && data.pack === 'pack_animal') ? data.score : 0 },
            { id: "w02", name: "ノーミスクリア", description: "ノーミスで3回クリアする", goal: 3, reward: 1500, check: (res, data) => (res.errors === 0) ? 1 : 0 },
            { id: "w03", name: "ウィークリーコンボ", description: "合計コンボ数 500 を達成", goal: 500, reward: 1000, check: (res, data) => data.maxCombo },
            { id: "w04", name: "美食家", description: "食べ物パックで合計10,000点稼ぐ", goal: 10000, reward: 1000, check: (res, data) => (data.gameMode === 'time_30' && data.pack === 'pack_food') ? data.score : 0 }
        ];
        
        // ★イベント
        const masterEventList = [
            { id: "e_sakura", name: "桜まつりイベント", start: "04-01", end: "04-15",
              description: "「桜まつり」開催中！<br>・背景「桜」が全員に解放！<br>・限定単語「さくら」「おはなみ」が出現！<br>・ガチャから限定アイテム「桜のかんざし」排出！" }
        ];

        // ★実績
        const masterAchievementList = {
            totalTypes1000: { name: "タイパー見習い", description: "合計タイプ数 1,000回 達成", reward: 500, ap: 5, check: (data) => data.stats.totalTypes >= 1000 },
            totalTypes10000: { name: "ベテランタイパー", description: "合計タイプ数 10,000回 達成", reward: 2000, ap: 10, check: (data) => data.stats.totalTypes >= 10000 },
            totalTypes100000: { name: "タイプマスター", description: "合計タイプ数 100,000回 達成", reward: 10000, ap: 50, check: (data) => data.stats.totalTypes >= 100000 },
            maxCombo50: { name: "コンボマスター", description: "最大コンボ 50 達成", reward: 1000, ap: 10, check: (data) => data.stats.maxCombo >= 50 },
            maxCombo100: { name: "コンボ神", description: "最大コンボ 100 達成", reward: 5000, ap: 20, check: (data) => data.stats.maxCombo >= 100 },
            noMissTime30: { name: "パーフェクト", description: "30秒モードをノーミスでクリア", reward: 1500, ap: 15, check: (data, res) => res.gameMode === 'time_30' && res.errors === 0 },
            money10000: { name: "リッチ", description: "所持金 10,000 G 達成", reward: 1000, ap: 5, check: (data) => data.stats.money >= 10000 },
            money100000: { name: "大富豪", description: "所持金 100,000 G 達成", reward: 5000, ap: 20, check: (data) => data.stats.money >= 100000 },
            level10: { name: "レベル10到達", description: "プレイヤーレベルが10に到達する", reward: 1000, ap: 5, check: (data) => data.stats.level >= 10 },
            level50: { name: "レベル50到達", description: "プレイヤーレベルが50に到達する", reward: 5000, ap: 20, check: (data) => data.stats.level >= 50 },
            level100: { name: "レベル100到達", description: "プレイヤーレベルが100に到達する", reward: 10000, ap: 50, check: (data) => data.stats.level >= 100 },
            challengeTA: { name: "タイムアタッカー", description: "チャレンジ「TA (20単語)」をクリア", reward: 2000, ap: 15, check: (data, res) => res.gameMode === 'timeAttack' && res.errors === 0 },
            challengeAcc: { name: "精密機械", description: "チャレンジ「精度 (1分)」をクリア", reward: 2000, ap: 15, check: (data, res) => res.gameMode === 'accuracy' && res.accuracy >= 98.0 },
            prestige1: { name: "転生者", description: "初めてプレステージを行う", reward: 10000, ap: 100, check: (data) => data.stats.prestigeLevel >= 1 },
            prestige20: { name: "超越者", description: "プレステージレベル 20 に到達", reward: 50000, ap: 200, check: (data) => data.stats.prestigeLevel >= 20 },
            gacha10: { name: "ガチャ好き", description: "ガチャを10回引く", reward: 1000, ap: 5, check: (data) => data.stats.gachaPulls >= 10 },
            gachaSuperRare: { name: "強運", description: "スーパーレアを引く", reward: 5000, ap: 20, check: (data, res) => res.gachaRarity === 'super-rare' },
            gachaLegendary: { name: "伝説の引き", description: "レジェンダリーを引く", reward: 10000, ap: 50, check: (data, res) => res.gachaRarity === 'legendary' },
            skillMaster: { name: "スキルマスター", description: "スキルを10個アンロック", reward: 5000, ap: 20, check: (data) => Object.keys(data.skills).length >= 10 },
            petLover: { name: "ペット愛好家", description: "ペットを3匹アンロック", reward: 5000, ap: 10, check: (data) => data.unlockedPets.length >= 3 },
            newModeMaster: { name: "チャレンジャー", description: "新しいチャレンジモードを全てプレイ", reward: 3000, ap: 15, check: (data) => data.stats.modesPlayed['english'] && data.stats.modesPlayed['double'] && data.stats.modesPlayed['fill'] && data.stats.modesPlayed['fast'] }
        };
        ...
        const masterCharmList = {
            'none': { name: "なし", description: "お守りを装備していません。", effect: {} },
            'charm_maneki': { name: "金の招き猫", description: "獲得Gが永久に+5%されます。", effect: { money: 0.05 } },
            'charm_kame': { name: "亀の甲羅", description: "最初の3ミスまでコンボが途切れません。", effect: { shield: 3 } },
            'charm_taka': { name: "鷹の羽", description: "ポップエフェクトが少し豪華になります。", effect: { pop: 1 } },
            'charm_hude': { name: "達人の筆", description: "WPMに応じてスコアにボーナスが入ります。", effect: { wpmScore: 0.1 } }
        };

        // ★スキルツリー
        const masterSkillTree = {
            's01_score': { name: "スコアUP", icon: "S", maxLevel: 5, cost: 1, desc: "基礎スコアが [L*1]% 増加する", x: 50, y: 50, req: [] },
            's02_money': { name: "所持金UP", icon: "G", maxLevel: 5, cost: 1, desc: "獲得Gが [L*1]% 増加する", x: 150, y: 50, req: [] },
            'c01_combo': { name: "コンボ猶予", icon: "C", maxLevel: 3, cost: 2, desc: "コンボの猶予時間が [L*0.1]秒 増加する", x: 50, y: 150, req: ['s01_score'] },
            'c02_shield': { name: "シールド強化", icon: "H", maxLevel: 2, cost: 3, desc: "コンボシールドが [L*1]回 追加で発動する", x: 150, y: 150, req: ['s02_money'] },
            'f01_fever': { name: "フィーバー+", icon: "F", maxLevel: 3, cost: 2, desc: "フィーバー突入コンボ数が [L*1] 減少する", x: 50, y: 250, req: ['c01_combo'] },
            'f02_fever_rate': { name: "フィーバー倍率", icon: "F+", maxLevel: 2, cost: 3, desc: "フィーバー中のスコア倍率が [L*0.5]倍 増加する", x: 150, y: 250, req: ['c02_shield'] },
            'p01_pet': { name: "ペット応援", icon: "P", maxLevel: 1, cost: 5, desc: "ペットの応援がスコアに影響するようになる", x: 100, y: 350, req: ['f01_fever', 'f02_fever_rate'] },
            'g01_gacha': { name: "ガチャ運", icon: "R", maxLevel: 5, cost: 2, desc: "ガチャのレア排出率が [L*0.2]% 増加する (富豪転生と重複)", x: 250, y: 50, req: [] },
            'g02_gacha_cost': { name: "ガチャ割引", icon: "G-", maxLevel: 3, cost: 3, desc: "ガチャのコストが [L*5]% 減少する", x: 250, y: 150, req: ['g01_gacha'] },
        };
        ...
        // --- 5. セーブ/ロード機能 (Base64暗号化) ---
        let gameData;

        // ★バグ修正: saveData 関数を loadData の前に定義
        function saveData() {
            try {
                localStorage.setItem(STORAGE_KEY, btoa(JSON.stringify(gameData)));
            } catch (e) {
                console.error("Failed to save data:", e);
                // データを保存できないとゲームが進行しないため、あえてアラートは出さない
            }
        }
        ...
        function getXPForLevel(level) {
            ...
            if (gameData.stats.prestigeType === 'standard') {
                prestigeBonus = 1 - (gameData.stats.prestigeLevel * 0.05); // 1転生で5%減 (最大50%まで)
            }
        ...
        function getPrestigeStar(level) {
            if (level === 0) return "";
            const icon = (gameData.stats.prestigeType === 'wealth') ? '💎' : '★';
            return `${icon}${level}`; // ★20段階対応
        }
        ...
        function loadData() {
                        ...
                        for (const key in source) {
                            if (target[key] === undefined) {
                                ...
                                if (typeof target[key] !== 'object' || target[key] === null) {
                                    target[key] = source[key]; // 既存データがオブジェクトでない場合は上書き
                                } else {
                                    checkAndMerge(target[key], source[key]); // 再帰的にチェック
                                }
                            }
                        }
                    };
            ...
            // 今週のIDチェック (ウィークリーミッション)
            const weekId = getWeekId();
            if (gameData.missions.lastWeekId !== weekId) {
                gameData.missions.weekly = {}; // リセット
                gameData.missions.weeklyRewards = {}; // リセット
                // 新しいミッションを3つ設定
            ...
            applyPet();
            applyKeyboardSkin();
            applyCharmEffects(); // お守り効果を計算
            updateDataUI();

            prestigeBtn.style.display = (gameData.stats.level >= MAX_LEVEL && gameData.stats.prestigeLevel < MAX_PRESTIGE_LEVEL) ? 'flex' : 'none';
            
            checkActiveEvents();
            saveData(); // ★バグ修正: 読み込み直後に一度保存
        }// ★バグ修正: デフォルトアイテム未解放＆機能削除のため、getDefaultGameData を上書き定義
        function getDefaultGameData() { 
            let defaultItems = {};
            for (const itemKey in masterItemList) {
                if (!masterItemList[itemKey].gacha && !masterItemList[itemKey].event) { //★イベントアイテムを除外
            ...
            return { 
                ...
                // ★デフォルト解放バグ修正: デフォルトアイテムをリストに含める
                unlockedBackgrounds: ['bg-default'],
                unlockedFonts: ['font-default'], 
                unlockedSounds: ['sound-default'], 
                unlockedPacks: ['default', 'pack_english'], // 英語はデフォルト解放
                unlockedEffects: ['default'],
                unlockedThemes: ['default'], 
                unlockedPets: ['pet-cat'],
                unlockedPetItems: ['none'],
        ...
        function updateDataUI() {
            ...
            const modeLabels = {
                'time_30': "T-30ハイスコア",
                'words_30': "W-30ハイスコア",
                'suddenDeath': "サドンデス ハイスコア",
                'timeAttack': "TA (20単語) ハイスコア",
                'accuracy': "精度 (1分) ハイスコア",
                // 'lyrics': "歌詞タイピング ハイスコア", // ★歌詞削除
                'english': "英単語 ハイスコア",
                'double': "ダブル ハイスコア",
                'fill': "穴埋め ハイスコア",
                'fast': "早送り ハイスコア",
            ...
            let label = modeLabels[currentModeKey] || "ハイスコア";
        ...
        function loadCurrentWord() {
            ...
            if (gameMode === 'time' || gameMode === 'suddenDeath' || gameMode === 'accuracy' || gameMode === 'fast' || gameMode === 'english' || gameMode === 'fill') { 
                ...
                wordData = getNextWord();
                doubleWordData = getNextWord(); // 2つ目のお題
                currentWordIndex++;
            }
            ...
            if (gameMode === 'fill') {
                ...
                if (yomi.length > 2) {
                    ...
                    const displayYomi = yomi.substring(0, hideIndex) + '？' + yomi.substring(hideIndex + 1);
        ...
        function updateTargetRoma() {
            ...
            else if (targetHiragana === 'っ') {
                romajiOptions = romaMap['っ'].slice();
        ...
        function startGame() {
            ...
            fastModeSpeed = 1.0; // 早送りモードリセット
            
            // モードごとのお題設定
            if (gameMode === 'words' || gameMode === 'timeAttack') { // ★歌詞削除
                wordsToType = [];
                ...
                // wordsToType は switchMode 時に設定済み
            }
        ...
        function endGame() {
            ...
            clearInterval(timer); 
            stopGhostTimer(); // ★ゴースト停止
            setPetState('idle'); // ★ペット
            ...
            if (gameMode === 'timeAttack') finalScore = duration; // 時間
            if (gameMode === 'accuracy') finalScore = accuracy; // 正解率
            ...
            if (escapePressed) {
                document.getElementById('end-result-title').innerText = "お疲れ様";
                document.getElementById('end-title').innerText = "タイピング頑張れ！";
            } else {
                ...
                document.getElementById('end-title').innerText = newHighScore ? "🎉 NEW HIGH SCORE! 🎉" : "";
                if (newTitleUnlocked) { document.getElementById('end-title').innerText = `🏆 称号「${newTitleUnlocked}」獲得！ 🏆`; }
            }
        ...
        function getModeKey() {
            ...
            return gameMode; // 他はそのまま
        }

        function resetGame() {
            // loadData() は呼ばない。設定変更時のみ呼ぶ
            clearInterval(timer);
            stopGhostTimer(); 
            ...
            if (today.getDay() === 0 || today.getDay() === 6) {
                isWeekendBoost = true;
                effectText.push("週末ボーナス (スコア/G 1.5倍)");
            }
            
            if (gameData.activeItems.scoreBoost) { isScoreBoostActive = true; gameData.activeItems.scoreBoost = false; effectText.push("スコアx2"); }
            if (gameData.activeItems.moneyBoost) { isMoneyBoostActive = true; gameData.activeItems.moneyBoost = false; effectText.push("お金x2"); }
            ...
            if (shieldCount > 0) {
                isComboShieldActive = true; // 統合
                effectText.push(`シールド(${shieldCount})`);
            }
            
            if (gameData.activeItems.easyStart) { isEasyStartActive = true; gameData.activeItems.easyStart = false; effectText.push("イージースタート"); }
            if (gameData.activeItems.slowTimer) { isSlowTimerActive = true; gameData.activeItems.slowTimer = false; effectText.push("スロータイマー"); }
            if (gameData.activeItems.feverBoost) { isFeverItemActive = true; gameData.activeItems.feverBoost = false; effectText.push("フィーバー+"); } 

            
            let baseTime = 30; let baseWords = 30; 

            if (gameData.activeItems.boostStart) { currentCombo = 10; comboDisplay.innerText = 10; gameData.activeItems.boostStart = false; effectText.push("コンボ10スタート"); }
            if (gameData.activeItems.timePlusTwo) { baseTime += 2; gameData.activeItems.timePlusTwo = false; effectText.push("時間+2秒"); }
            if (gameData.activeItems.timePlusFive) { baseTime += 5; gameData.activeItems.timePlusFive = false; effectText.push("時間+5秒"); }
            if (gameData.activeItems.wordSkip) { baseWords -= 1; gameData.activeItems.wordSkip = false; effectText.push("ノルマ-1"); }
            
            if (effectText.length > 0) { 
                document.getElementById('item-effect-display').innerText = `（${effectText.join(" | ")} 発動中！）`; 
                saveData();
            } else { 
            ...
            const modeConfig = {
                ...
                // 'lyrics': { value: wordPacks['pack_lyrics'].length, timerText: "0.00", label: "TIME", words: [...wordPacks['pack_lyrics']] }, // ★歌詞削除
            ...
            wordDisplayElement.innerHTML = '<span style="font-size: 0.8em; color: #777;">[Space] でスタート</span>';
            wordDisplaySub.style.display = 'none';
        ...
        function processTypedKey(typedKey) {
            ...
            // ★ダブルモード: 2つのお題のどちらかにマッチするか判定
            if (gameMode === 'double' && doubleWordData) {
                const yomi1 = hiraganaSpans.map(s => s.innerText).join('');
                const yomi2 = doubleWordData.yomi;

                if (typedKey === yomi1[0]) { // 1つ目のお題
                    // そのまま進む
                } else if (typedKey === yomi2[0]) { // 2つ目のお題に切り替え
                    // 現在のお題をサブに
                    ...
                    loadCurrentWord(); // メインを再読み込み
                    processTypedKey(typedKey); // キー入力を再処理
                    return;
                }
            }
            ...
            if (targetRomaOptions.includes(newRomaInput)) {
                ...
                if (targetHiraganaIndex >= hiraganaSpans.length) {
                    ...
                    if (gameMode === 'fast') {
                        fastModeSpeed *= 1.05; // 5%ずつ加速
                    }

                    if (gameMode === 'words' || gameMode === 'custom' || gameMode === 'timeAttack') { // ★歌詞削除
                        currentWordIndex++; 
                    }
                    loadCurrentWord();
                } else { 
        ...
        function addXP(amount) {
            ...
            if (gameData.stats.level >= MAX_LEVEL) return; // 転生待ち
            ...
            if (newLevel > currentLevel) {
                for (let lv = currentLevel + 1; lv <= newLevel; lv++) {
                    gameData.stats.level = Math.min(lv, MAX_LEVEL); // Lv100でストップ
                    playSound('levelup');
                    const bonus = lv * 100; 
                    ...
                    if (lv % 5 === 0) {
                        ...
                        showQuickClaim(`レベルアップ！ Lv. ${gameData.stats.level}！\n+${bonus} G / +1 SP 獲得！`);
                    } else {
                         showQuickClaim(`レベルアップ！ Lv. ${gameData.stats.level}！\n+${bonus} G 獲得！`);
                    }
                    ...
                    if (gameData.prestigeTrial.active && gameData.stats.level >= 30) {
                        failPrestigeTrial("Lv. 30に到達してしまいました");
                    }
                }
        ...
        function handleMiss() {
            ...
            if (errors < shieldCount) {
                errors++; // シールド消費
                document.getElementById('item-effect-display').innerText = `シールド発動！ (残り ${shieldCount - errors}回)`;
                playSound('shield');
                currentRomaInput = "";
                if(hiraganaSpans[targetHiraganaIndex]) { 
        ...
        }// --- 9. モーダル (ショップ/称号/設定/ミッション/実績) の制御 ---
        ...
        function renderShop() {
            ...
            const itemUnlockLevels = {
                ...
                'pack_difficult': 20, // ★神々パック
            };

            Object.keys(masterItemList).forEach(itemId => {
                const item = masterItemList[itemId];
                if (item.gacha || item.event) return; // ガチャ限定品とイベント品はショップに表示しない
                ...
                if (requiredLevel && currentLevel < requiredLevel) {
                    buttonHtml = `<button class="item-buy-btn" disabled>Lv. ${requiredLevel} で解放</button>`;
                } else {
                    ...
                    if (unlockedList) {
                        if (unlockedList.includes(itemId)) {
                            buttonHtml = `<button class="item-buy-btn active" disabled>購入済み</button>`;
                        } else if (gameData.stats.money >= item.price) {
                            buttonHtml = `<button class="item-buy-btn" onclick="buyItem('${itemId}')">購入 (${item.price.toLocaleString()}G)</button>`;
                        } else {
                            buttonHtml = `<button class="item-buy-btn" disabled>資金不足 (${item.price.toLocaleString()}G)</button>`;
                        }
                        ...
                        buttonHtml = `<button class="item-buy-btn active" disabled>使用待ち</button>`;
                    } else if (gameData.stats.money >= item.price) {
                        buttonHtml = `<button class="item-buy-btn" onclick="buyItem('${itemId}')">購入 (${item.price.toLocaleString()}G)</button>`;
                    } else {
                        buttonHtml = `<button class="item-buy-btn" disabled>資金不足 (${item.price.toLocaleString()}G)</button>`;
            ...
            if (gachaBonus > 0) {
                gachaBonusText.innerText = ` (レア率 +${gachaBonus.toFixed(1)}%UP中!)`;
            }
        }
        window.buyItem = function(itemId) {
            ...
            if (itemLists[item.type] && itemLists[item.type].includes(itemId)) {
                return alert("このアイテムは購入済みです。");
            }
            if (item.type === 'upgrade' && (gameData.upgrades[item.key] || 0) > 0) return alert("このアップグレードは購入済みです。");
            if (item.type === 'boost' && gameData.activeItems[itemId]) return alert("このアイテムは既に使用待ちです。");
            ...
            if (gameData.stats.money >= item.price) {
                ...
                if (item.type === 'utility') {
                    if (itemId === 'bonusMoney') {
                        ...
                        alert(`${item.name} を購入しました！ 所持金が50円増えます。`);
                    }
        ...
        function renderTitleList() {
            ...
            masterTitleList.forEach((title, index) => {
                ...
                const statusText = isUnlocked ? '✅ 解放済み' : '🔒 未解放';
                titleEl.innerHTML = `<span class="item-name">${title.name}</span> <span class="item-description">必要スコア: ${title.score.toLocaleString()}</span> <span class="${statusClass}">${statusText}</span>`;
        ...
        function renderWeeklyMissions() {
            ...
            weeklyTimer.innerText = `残り ${days}日 ${hours}時間`;
        ...
        function renderAchievementList() {
            ...
            Object.keys(masterAchievementList).forEach(key => {
                ...
                const statusClass = isUnlocked ? 'item-status unlocked' : 'item-status locked';
                const statusText = isUnlocked ? '✅ 達成済み' : '🔒 未達成';
        ...
        function claimApReward(index, reward) {
            ...
            if (reward.type === 'money') {
                ...
                rewardMsg = `${reward.value.toLocaleString()} G を獲得しました！`;
            } else if (reward.type === 'item') {
                gameData.activeItems[reward.value] = true;
                rewardMsg = `${masterItemList[reward.value].name} を獲得しました！`;
        ...
        function checkWeeklyMissions(gameResult) {
            gameData.missions.currentWeeklyIds.forEach(id => {
                if (gameData.missions.weeklyRewards[id]) return; // 既に達成済み
        ...
        function showQuickClaim(message) {
            ...
            if (quickClaimQueue.length === 1) { // 最初の通知なら表示
        ...
        function displayNextQuickClaim() {
            ...
            playSound('reward'); // ★通知音
        }
        
        quickClaimBtn.addEventListener('click', () => {
            quickClaimQueue.shift(); // 最初のメッセージをキューから削除
            displayNextQuickClaim(); // 次のメッセージを表示
        });
        ...
        function renderStats() {
            ...
            const playTimeStr = `${hours}時間 ${minutes}分 ${seconds}秒`;
            ...
            const prestigeType = stats.prestigeType === 'wealth' ? '富豪' : (stats.prestigeType === 'standard' ? '標準' : 'なし');

            statsContainer.innerHTML = `
                <div class="stat-block">
                    <div class="stat-value">${stats.level} ${prestigeStar}</div>
                    <div class="stat-label">現在レベル (転生タイプ: ${prestigeType})</div>
                </div>
                <div class="stat-block">
                    <div class="stat-value">${stats.ap.toLocaleString()}</div>
                    <div class="stat-label">累計実績ポイント (AP)</div>
                </div>
                <div class="stat-block">
                ...
                <div class="stat-block">
                    <div class="stat-value">${stats.totalGamesPlayed.toLocaleString()} <span style="font-size: 0.5em;">回</span></div>
                    <div class="stat-label">総プレイ回数</div>
                </div>
                <div class="stat-block">
                    <div class="stat-value">${stats.totalTypes.toLocaleString()}</div>
                    <div class="stat-label">累計タイプ回数</div>
                </div>
                <div class="stat-block">
                ...
                <div class="stat-block">
                    ...
                    <div class="stat-label">平均 正解率</div>
                </div>
            `;
                ...
                options: {
                    ...
                    scales: {
                        ...
                        yScore: {
                            ...
                            ticks: { color: '#ecf0f1' },
                            grid: { drawOnChartArea: false } //片方だけ
                        },
                        x: {
                            ticks: { color: '#ecf0f1' },
                            grid: { color: 'rgba(255,255,255,0.1)' }
        ...
        function applyFuriganaSetting() {
            ...
            if (gameData.settings.showFurigana) {
                ...
                btn.innerHTML = '<i class="fas fa-eye"></i> ふりがな: ON';
            } else {
                document.body.classList.remove('show-furigana');
                btn.innerHTML = '<i class="fas fa-eye-slash"></i> ふりがな: OFF';
            }
        }
        ...
        function applyBGMSetting(userClicked = false) {
            ...
            if (gameData.settings.bgmOn) {
                ...
                // currentBgm.muted = false; // playBGM内で処理
        ...
        function renderFontSelector() {
            ...
            container.innerHTML = '';
            const allFonts = [
                { id: 'font-default', name: 'デフォルト' },
                { id: 'font_serif', name: '明朝体' },
                { id: 'font_rounded', name: '丸ゴシック' },
                { id: 'font_pixel', name: 'ピクセル (テーマ)' }
            ];
            ...
            allFonts.forEach(font => {
                ...
                if (gameData.unlockedFonts.includes(font.id) || gameData.unlockedFonts.includes(fontId)) { // ★バグ修正
        ...
        function renderSoundSelector() {
            ...
            container.innerHTML = '';
            const allSounds = [
                { id: 'sound-default', name: 'デフォルト (Pop)' },
                { id: 'sound_click', name: 'カチカチ' },
                { id: 'sound_synth', name: 'ピコピコ' }
            ];
            ...
            allSounds.forEach(sound => {
                ...
                if (gameData.unlockedSounds.includes(sound.id) || gameData.unlockedSounds.includes(soundId)) { // ★バグ修正
        ...
        function renderWordPackSelector() {
            ...
            const allPacks = [
                ...
                { id: 'pack_animal', name: '動物パック' },
                { id: 'pack_food', name: '食べ物パック' },
                { id: 'pack_difficult', name: '神々の単語' }
            ];
            ...
            allPacks.forEach(pack => {
                ...
                if (gameData.unlockedPacks.includes(pack.id) || gameData.unlockedPacks.includes(packId)) { // ★バグ修正
        ...
        function renderEffectSelector() {
            ...
            const allEffects = [
                { id: 'default', name: 'デフォルト' },
                { id: 'effect_rainbow', name: '虹色' },
                { id: 'effect_firework', name: '花火' },
                { id: 'effect_bubble', name: '泡' },
                { id: 'effect_lines', name: '集中線' },
                { id: 'effect_gold', name: 'ゴールド (ガチャ限定)' }
            ];
            ...
            allEffects.forEach(effect => {
                ...
                if (gameData.unlockedEffects.includes(effect.id) || gameData.unlockedEffects.includes(effectId)) { // ★バグ修正
        ...
        function applyTheme() {
            ...
            document.body.className = ''; // まず全クラスをリセット
            if (theme !== 'default') {
        ...
        function renderThemeSelector() {
            ...
            const allThemes = [
                { id: 'default', name: 'デフォルト' },
                { id: 'theme_cyber', name: 'サイバー' },
                { id: 'theme_wa', name: '和風' },
                { id: 'theme_pixel', name: 'ピクセル' }
            ];
            allThemes.forEach(theme => {
                ...
                if (gameData.unlockedThemes.includes(theme.id) || gameData.unlockedThemes.includes(themeId)) { // ★バグ修正
        ...
        function renderBackgroundSelector() {
            ...
            container.innerHTML = '';
            const allBgs = [
                { id: 'bg-default', name: 'デフォルト' },
                { id: 'bg_ocean', name: '海' },
                { id: 'bg_galaxy', name: '銀河' },
                { id: 'bg_forest', name: '森' },
                { id: 'bg_sunset', name: '夕焼け' },
                { id: 'bg_cyber', name: 'サイバー' },
                { id: 'bg_legend', name: '伝説' },
                { id: 'bg_aurora', name: 'オーロラ (ガチャ限定)' },
                { id: 'bg_sakura', name: '桜 (イベント限定)' }
            ];
            ...
            allBgs.forEach(bg => {
                ...
                const bgId = bg.id.replace('bg_', 'bg-'); // ★バグ修正: ID統一
                
                if (gameData.unlockedBackgrounds.includes(bg.id) || gameData.unlockedBackgrounds.includes(bgId)) { // ★バグ修正
                    if (gameData.settings.currentBackground === bg.id || gameData.settings.currentBackground === bgId) { // ★バグ修正
                        btn.classList.add('active');
                    }
                    btn.addEventListener('click', () => {
                        gameData.settings.currentBackground = bg.id; // 保存は_付き
                        applyBackground();
                        saveData();
            ...
            if (gameData.settings.currentBackground === 'bg_legend') {
                ...
                effectBtn.innerHTML = gameData.settings.legendRainbowEffect 
                    ? '<i class="fas fa-magic"></i> 伝説エフェクト: ON' 
                    : '<i class="fas fa-magic"></i> 伝説エフェクト: OFF';
            } else {
        ...
        function applyPet() {
            ...
            const petEmojis = {'cat': '😺', 'dog': '🐶', 'robot': '🤖'};
            petSprite.innerText = petEmojis[petId] || '😺';
            petSprite.className = `idle`;
            
            const itemId = gameData.settings.currentPetItem;
            const itemEmojis = {'none': '', 'pet_item_santa': '🎅', 'pet_item_party': '🥳', 'pet_item_sakura': '🌸'}; // ★ID修正
            petItemHat.innerText = itemEmojis[itemId] || '';

            const houseId = gameData.settings.currentPetHouse;
            const houseEmojis = {'pet-house-none': '', 'pet_house_cushion': '🛋️', 'pet_house_space': '🚀'}; // ★ID修正
        ...
        function renderPetSelector() {
            ...
            const allPets = [
                { id: 'pet-cat', name: '猫' },
                { id: 'pet_dog', name: '犬' }, // ★ID修正
                { id: 'pet_robot', name: 'ロボット (AP報酬)' } // ★ID修正
            ];
            allPets.forEach(pet => {
                const btn = document.createElement('button');
                ...
                if (gameData.unlockedPets.includes(pet.id) || gameData.unlockedPets.includes(petId)) { // ★バグ修正
        ...
        function renderPetItemSelector() {
            ...
            const allItems = [
                { id: 'none', name: 'なし' },
                { id: 'pet_item_santa', name: 'サンタ帽' },
                { id: 'pet_item_party', name: 'パーティ帽 (ガチャ)' },
                { id: 'pet_item_sakura', name: '桜のかんざし (イベ)' }
            ];
            allItems.forEach(item => {
                ...
                if (gameData.unlockedPetItems.includes(item.id)) { // ★ID修正
        ...
        function renderPetHouseSelector() {
            ...
            container.innerHTML = '';
            const allItems = [
                { id: 'pet-house-none', name: 'なし' },
                { id: 'pet_house_cushion', name: 'クッション' }, // ★ID修正
                { id: 'pet_house_space', name: '宇宙船' } // ★ID修正
            ];
            allItems.forEach(item => {
                const btn = document.createElement('button');
                ...
                if (gameData.unlockedPetHouses.includes(item.id) || gameData.unlockedPetHouses.includes(itemId)) { // ★バグ修正
        ...
        function applyKeyboardSkin() {
            ...
            keyboardContainer.className = ''; // リセット
        ...
        function renderKeyboardSkinSelector() {
            ...
            container.innerHTML = '';
            const allSkins = [
                { id: 'kb-skin-default', name: 'デフォルト' },
                { id: 'kb_skin_wood', name: '木目調' },
                { id: 'kb_skin_pixel', name: 'ピクセル' },
                { id: 'kb_skin_neko', name: '肉球 (ガチャ)' }
            ];
            allSkins.forEach(skin => {
                ...
                if (gameData.unlockedKeyboardSkins.includes(skin.id) || gameData.unlockedKeyboardSkins.includes(skinId)) { // ★バグ修正
                    if (gameData.settings.currentKeyboardSkin === skin.id || gameData.settings.currentKeyboardSkin === skinId) btn.classList.add('active');
                    btn.addEventListener('click', () => {
                        gameData.settings.currentKeyboardSkin = skin.id; // _付きで保存
                        applyKeyboardSkin();
                        saveData();
        ...
        function renderBgmSelector() {
            ...
            allBgmsList.forEach(bgm => {
                ...
                if (gameData.unlockedBgms.includes(bgm.id) || gameData.unlockedBgms.includes(bgmId)) { // ★バグ修正
        ...
        function applyCharmEffects() {
            // ★お守りバグ修正: ゲームリセット時に効果を適用するため、resetGame() から呼び出される
        }
        ...
        // document.getElementById('lyrics-mode-btn').addEventListener('click', () => switchMode('lyrics')); // ★歌詞削除
        ...
        document.addEventListener('keydown', (e) => {
            ...
            const isModalOpen = [
                ...
                adminPanelOverlay, gachaResultOverlay, // ★ログインボーナス削除
        ...
        const overlaysAndCloseBtns = {
            ...
            // 'login-bonus-overlay': 'close-login-bonus-btn', // ★ログインボーナス削除
        ...
        document.getElementById('delete-data-btn').addEventListener('click', () => {
            if (window.confirm("本当にすべてのデータを削除しますか？\n（ハイスコア、所持金、レベル、解放した全アイテムがリセットされます）")) {
                try {
                    ...
                    resetGame(); 
                } catch (e) { console.error("Data deletion failed", e); alert("データの削除に失敗しました。"); }
            }
        });
        profileChangeNameBtn.addEventListener('click', () => {
            const name = prompt("ランキングに登録する名前を入力してください:", gameData.stats.playerName);
            if (name && name.trim().length > 0) {
                gameData.stats.playerName = name.trim();
                saveData();
                renderProfile();
                alert("名前を変更しました: " + gameData.stats.playerName);
            }
        });
        document.getElementById('pop-mode-btn').addEventListener('click', () => { 
            if (popMode === 'random') {
                popMode = 'fixed';
                document.getElementById('pop-mode-btn').innerHTML = '<i class="fas fa-bullseye"></i> 演出: 入力箇所';
            } else {
                popMode = 'random';
                document.getElementById('pop-mode-btn').innerHTML = '<i class="fas fa-bolt"></i> 演出: ランダム';
            }
        });
        ...
        weakWordPracticeBtn.addEventListener('click', () => {
            ...
            const weakWords = Object.values(gameData.weakWords)
                .filter(w => w.miss > 0 && w.total > 2) // 3回以上挑戦してミスした単語
                .sort((a, b) => (b.miss / b.total) - (a.miss / a.total)); // ミス率順
            ...
            wordsToType = weakWords.slice(0, 20); // 上位20件
            gameValue = wordsToType.length; 
            settingsOverlay.style.display = 'none'; 
            switchMode('custom');
            startGame();
        ...
        prestigeBtn.addEventListener('click', () => {
            if (gameData.stats.level < MAX_LEVEL) {
                alert(`レベル ${MAX_LEVEL} でプレステージが解放されます。`);
                return;
            }
            if (gameData.stats.prestigeLevel >= MAX_PRESTIGE_LEVEL) {
                alert("あなたは既に最大転生レベルに達しています。");
                return;
            }
            ...
            if (gameData.stats.prestigeType === null) {
                ...
                if (gameData.prestigeTrial.active) {
                    const trialMap = { 'combo': 'コンボの試練', 'perfect': '完全無欠の試練' };
                    prestigeCurrentTrialText.innerText = `${trialMap[gameData.prestigeTrial.type]} (達成度: ${gameData.prestigeTrial.progress}%)`;
                    prestigeCurrentTrialText.style.color = 'var(--color-accent)';
                } else {
                    prestigeCurrentTrialText.innerText = "なし (試練失敗、または未選択)";
        ...
        prestigeTrialNone.addEventListener('click', () => {
             if (window.confirm("試練なしで転生しますか？")) {
                executePrestige();
                prestigeChoiceOverlay.style.display = 'none';
            }
        });
        prestigeTrialCombo.addEventListener('click', () => {
             if (window.confirm("試練「コンボの試練」を開始しますか？")) {
        ...
        prestigeTrialPerfect.addEventListener('click', () => {
             if (window.confirm("試練「完全無欠の試練」を開始しますか？")) {
        ...
        function executePrestige() {
            ...
            let spGained = 0;
            if (gameData.prestigeTrial.active) {
                if (gameData.prestigeTrial.progress >= 100) { // 達成
                    spGained = gameData.prestigeTrial.rewardSP;
                    alert(`試練達成！ スキルポイント ${spGained} SP を獲得！`);
                } else {
                    // 試練失敗 (SPなし)
                }
            } else {
            ...
            gameData.prestigeTrial = { active: false, type: null, rewardSP: 0, progress: 0 }; // 試練リセット
            ...
            loadData();
            resetGame();
            alert(`プレステージ ${gameData.stats.prestigeLevel} に到達しました！`);
            settingsOverlay.style.display = 'none';
        }
        ...
        function checkPrestigeTrial(gameResult) {
            ...
            if (trial.progress >= 100) {
                alert("プレステージの試練を達成しました！\nLv. 100 に到達して転生すると、追加SPがボーナスされます。");
                trial.active = false; // 達成したらフラグはオフにする (報酬は保持)
                saveData();
            }
        }
        ...
        gachaPullBtn.addEventListener('click', () => {
            ...
            if (!window.confirm(`ランダムボックスを引きますか？\n(${cost.toLocaleString()} G)`)) {
                return;
            }
            ...
            prestigeBonus += getSkillEffect('g01_gacha') * 0.2; // スキルボーナス
            
            if (prestigeBonus > 0) {
                baseRates['legendary'] = Math.min(baseRates['legendary'] + (prestigeBonus * 0.2), 20); // 2割増し (上限20%)
                baseRates['super-rare'] = Math.min(baseRates['super-rare'] + (prestigeBonus * 0.8), 50); // 8割増し (上限50%)
                baseRates['normal'] = Math.max(0, baseRates['normal'] - prestigeBonus); // ノーマルが減る
            }
            ...
            checkAchievements({ gachaRarity: rarity }); // ガチャ実績チェック
        ...
        function addRankingEntry(modeKey, score) {
            ...
            if (window.confirm("ハイスコアをランキングに登録しますか？")) {
                const name = prompt("ランキングに登録する名前を入力してください:", playerName);
                if (name && name.trim().length > 0) {
                    gameData.stats.playerName = name.trim();
        ...
        function renderRanking(modeKey = 'time_30') {
            ...
            const modes = [
                { key: 'time_30', name: '30秒' },
                { key: 'suddenDeath', name: 'サドンデス' },
                { key: 'timeAttack', name: 'TA' },
                { key: 'accuracy', name: '精度' },
                // { key: 'lyrics', name: '歌詞' }, // ★歌詞削除
                { key: 'english', name: '英単語' },
                { key: 'double', name: 'ダブル' },
                { key: 'fill', name: '穴埋め' },
                { key: 'fast', name: '早送り' }
            ];
            ...
            ranking.forEach((entry, index) => {
                ...
                const iconEmoji = (masterItemList[entry.icon] ? masterItemList[entry.icon].emoji : '👤') || '👤';
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (importedData && importedData.stats && importedData.highScores) {
                        if (window.confirm("セーブデータを読み込みます。現在のデータは上書きされます。\nよろしいですか？")) {
                            gameData = importedData;
                            saveData(); loadData(); resetGame();
                            alert("データを読み込みました。");
                            settingsOverlay.style.display = 'none';
                        }
                    } else { alert("無効なセーブデータファイルです。"); }
        ...
        // --- 11. 初期化実行 ---
        // ★バグ修正: loadData() と resetGame() をグローバルスコープで実行
        loadData();
        resetGame();
        ...
        document.getElementById('code-btn').addEventListener('click', () => {
            ...
            const inputCode = prompt("コードを入力してください:");
            ...
            if (inputCode === "MANY") {
                ...
                saveData();
                updateDataUI();
                alert(`所持金が ${amount.toLocaleString()} G 増えました！`);
            } 
        ...
        adminPanelContainer.addEventListener('click', (e) => {
            ...
            switch (action) {
                ...
                case 'unlock-all':
                    ...
                    lists.forEach((listKey) => {
                        ...
                        gameData[listKey] = [...new Set([...gameData[listKey], ...allItems])]; // ★バグ修正: デフォルト含めてマージ
                    });
                    alert('すべてのカスタマイズをアンロックしました。');
                    break;
                case 'lock-all':
                    const oldStats = { ...gameData.stats }; // 統計情報は維持
                    gameData = getDefaultGameData();
                    gameData.stats = oldStats; // 統計情報を戻す
                    alert('カスタマイズをデフォルト以外すべてロックしました。');
                    break;
                case 'unlock-titles':
                    gameData.unlockedTitles = Array(TOTAL_TITLES).fill(true);
                    alert('すべての称号をアンロックしました。');
                    break;
                case 'lock-titles':
                    gameData.unlockedTitles = Array(TOTAL_TITLES).fill(false);
                    alert('すべての称号をロックしました。');
                    break;
                default:
                    actionTaken = false;
                    break;
            ...
            if (actionTaken) {
                saveData();
                loadData(); // ★バグ修正: loadDataを呼んでUIに反映
                resetGame();
            }
        });
        ...
        document.getElementById('auto-type-toggle').addEventListener('click', (e) => {
            ...
            if (isAutoTyping) {
                e.target.innerText = "自動入力 ON";
                e.target.classList.add('active');
                startAutoTyper();
            } else {
                e.target.innerText = "自動入力 OFF";
        ...
        // --- 13. 新機能 (スキルツリー、プロフィール、お知らせ) ---
        
        function renderProfile() {
            profileMainIcon.innerText = (masterItemList[gameData.settings.currentIcon] ? masterItemList[gameData.settings.currentIcon].emoji : '👤') || '👤';
            profileName.innerText = gameData.stats.playerName;
            profileIcon.innerText = (masterItemList[gameData.settings.currentIcon] ? masterItemList[gameData.settings.currentIcon].emoji : '👤') || '👤';
            ...
            const currentTitle = titles.length > 0 ? titles[titles.length - 1].name : 'タイパー見習い';
            ...
            profileBonusList.innerHTML = `
                <div>スコアボーナス: <span>+${bonuses.score.toFixed(0)}%</span></div>
                <div>所持金ボーナス: <span>+${bonuses.money.toFixed(0)}%</span></div>
                <div>ガチャレア率UP: <span>+${bonuses.gacha.toFixed(1)}%</span></div>
                <div>フィーバー突入: <span>-${bonuses.fever} コンボ</span></div>
                <div>追加シールド: <span>+${bonuses.shield} 回</span></div>
            `;
        }
        ...
        function renderNewsBoard() {
            ...
            if (gameData.activeEvent) {
                ...
                el.innerHTML = `<h3><i class="fas fa-magic"></i> イベント開催中</h3> <p>${event.description}</p>`;
            ...
            weeklyEl.innerHTML = `<h3><i class="fas fa-calendar-week"></i> 今週のミッション</h3> <p>「ミッション」タブから挑戦できます。今週も頑張りましょう！</p>`;
            ...
            if (gameData.prestigeTrial.active) {
                const el = document.createElement('div');
                el.className = 'news-item';
                el.innerHTML = `<h3><i class="fas fa-tasks"></i> 転生の試練 挑戦中</h3> <p>現在、プレステージの試練に挑戦中です。Lv. 30になる前に達成しましょう！</p>`;
                newsListContainer.appendChild(el);
            }
        }
        ...
        function renderSkillTree() {
            ...
            for (const id in masterSkillTree) {
                ...
                node.addEventListener('mouseenter', (e) => {
                    const statusText = isLocked ? '前提スキルが必要です' : (currentLevel >= skill.maxLevel ? 'MAXレベルです' : (gameData.stats.skillPoints < skill.cost ? `SPが不足しています (コスト: ${skill.cost} SP)` : `クリックで習得 (コスト: ${skill.cost} SP)`));
                ...
                skill.req.forEach(reqId => {
                    ...
                    line.style.left = `${reqSkill.x + 30}px`; // ノードの中心から
        ...
        function unlockSkill(id) {
            ...
            applyCharmEffects(); // スキルによるお守り効果変更など
            renderSkillTree();
        }
        ...
        function showSkillTooltip(name, desc, cost, status) {
            ...
            document.getElementById('skill-tooltip-status').innerText = (status === 'locked') ? '前提スキルが必要です' : '';
        }
        ...
        skillResetBtn.addEventListener('click', () => {
            ...
            if (gameData.stats.money < cost) {
                alert(`スキルリセットには ${cost.toLocaleString()} G が必要です。`);
                return;
            }
            if (window.confirm(`本当にスキルをリセットしますか？\n消費したすべてのSPが戻ります。\n(コスト: ${cost.toLocaleString()} G)`)) {
                let totalSpSpent = 0;
                for (const id in gameData.skills) {
                    if (masterSkillTree[id]) { // ★バグ修正: 存在しないスキルIDを参照しない
        ...
        savePresetBtn.addEventListener('click', () => {
            ...
            if (window.confirm(`現在のスキルとお守りをプリセット ${presetId + 1} に上書き保存しますか？`)) {
                savePreset(presetId);
                alert(`プリセット ${presetId + 1} に保存しました。`);
            }
        });
        ...
        function loadPreset(id) {
            if (window.confirm(`プリセット ${id + 1} を読み込みますか？\n現在のスキル割り振りはリセットされます。\n(SPは戻ります)`)) {
                ...
                if (gameData.stats.skillPoints >= presetSpCost) {
                    ...
                    alert(`SPが足りません。${presetSpCost} SP が必要です。`);
                    // ロード失敗したのでリセット状態のまま
                    saveData();
                    renderSkillTree();
                }
